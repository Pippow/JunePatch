...
{
    [CardEffectId("70194")]//when you play a relict from hand, strengthen self by 1. Every 2 turns on turn end, if it doesn’t destroy itself, Halve this unit's base power, then Spawn a copy with the same base power on this row
    ...
    {//
        ...
        {//strengthen by 1 when you play a relict from hand
            ...
            if (@event.Target.CardInfo().CardId == "70195") return;
            if (PlayerIndex == @event.Target.PlayerIndex && Card.Status.CardRow.IsOnPlace() && @event.Target.HasAllCategorie(Categorie.Relict))
            {
                used = true;
                return;
            }
